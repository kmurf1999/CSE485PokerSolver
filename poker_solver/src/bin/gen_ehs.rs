...
use rust_poker::hand_range::{ HandRange, Combo };
...
fn generate_ehs_table() {
    ...
    let mut file = OpenOptions::new().write(true).create_new(true).open(EHS_TABLE_FILENAME).unwrap();
    ...
    for i in 0..ROUNDS {
            ...
            for (j, slice) in equity_table.chunks_mut(num_hands).enumerate() {
                scope.spawn(move |_| {
                    ...
                    for k in 0..slice.len() {
                        ...
                        if (j == 0) && (k & 0xfff == 0) {
                            print!("round {}: {:.3}% \r", i, (100 * k) as Precision / num_hands as Precision);
                            io::stdout().flush().unwrap();
                        }

                        // get hand at index
                        indexers[i].get_hand(round, (j * size_per_thread + k) as u64, cards.as_mut_slice());
                        // get hole cards
                        ...
                        slice[k] = match i {
                            ...
                            2 => calc_equity(&hand_ranges, board_mask, N_THREADS as u8, 10000)[0],
        ...
        println!("round {} done. took {}ms ({:.2} iterations / ms)", i, duration, num_hands as f64 / duration as f64);
    }
}
